...
The architecture proposal in `/tmp/chrote-comms-architecture-proposal.md` framed the right primitive: append-only event streams bound to work objects, projected into a Slack-shaped reading surface. Two standalone dogfoods outside `/srv/app` tested that framing:
