...
func TestParseChroteOutputsRepairsTerminalWrappedJSONString(t *testing.T) {
	_, outputs, err := parseChroteOutputs("{\n  \"port_report\": {\n    \"text\": \"done\",\n    \"ref\": \"/workspace/operator/.formations/artifacts/run_123/\n  agents-tab-audit.md\"\n  }\n}")
	if err != nil {
		t.Fatalf("parse wrapped outputs: %v", err)
	}
	if got, want := outputs["port_report"].Ref, "/workspace/operator/.formations/artifacts/run_123/agents-tab-audit.md"; got != want {
